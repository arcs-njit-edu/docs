...
!!! note
    ...
    The 300,000 SUs are avaialble on `--qos=standard` only. If PI does not want to buy more SUs, PI's group members can use `--qos=low` which does not hve any SU charges. For more details, see [SLURM QOS](slurm.md#using-SLURM-on-Wulver) 
