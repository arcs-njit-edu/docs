...
3. The `/research` and `$HOME` directory from [Lochness](lochness.md) will be mounted on Wulver as `/research` and `/oldhome` respectively. For details see [Migration](lochness_filesystem.md)

```python exec="on"
import pandas as pd

df = pd.read_csv('docs/assets/tables/Wulver_filesystems.csv')
